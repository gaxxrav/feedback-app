...
const CreateFeedback = () => {
  ...
  const fetchBoardsAndTags = async () => {
    try {
      const [boardsResponse, tagsResponse] = await Promise.all([
        api.get('/boards/'),
        api.get('/tags/')
      ]);
      ...
      setError('Failed to load boards and tags');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to the created feedback or back to the board
      if (boardId) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="mb-6">
        <Link
          to={boardId ? `/board/${boardId}` : '/feedback'}
          className="text-indigo-600 hover:text-indigo-800 mb-4 inline-block"
        >
          ← Back to {boardId ? 'Board' : 'Feedback'}
        </Link>
        
        <h1 className="text-2xl font-bold text-gray-900">Create Feedback</h1>
      </div>
